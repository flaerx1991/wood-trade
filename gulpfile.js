...
var source = './src/';
var destination = './public/libs/';
var admin_destination = './public/admin/libs/';
 ...
 */
 gulp.task('admin_sass', function ( ){
    gulp.src(source + '/admin_scss/style.scss')
        .pipe(plumber(plumberErrorHandler))
        .pipe(sourcemaps.init())
        .pipe(sass().on('error', sass.logError))
        .pipe(autoprefixer())
        .pipe(cleanCSS({compatibility: 'ie9'}))
        .pipe(sourcemaps.write('./'))
        .pipe(gulp.dest(admin_destination + '/css'))
        .pipe(browserSync.stream());
...
// gulp.task('scriptBuild', () => processScript(false));
// gulp.task('scriptWatch', () => processScript(true));

// function processScript(watchFlag) {
//     return gulp.src('./src/admin_js/*.js')
//       .pipe(named())
//       .pipe(webpack({
//         watch: watchFlag,
//         module: {
...
//       }))
//       .pipe(gulp.dest('./public/admin/libs/js'));
//   }

gulp.task('js-admin', function () {
    gulp.src(source + '/admin_js/**/*.js')
        .pipe(plumber(plumberErrorHandler))
        .pipe(sourcemaps.init())
        .pipe(uglify())
        .pipe(concat('custom.js'))
        .pipe(sourcemaps.write('./'))
        .pipe(gulp.dest(admin_destination + '/js'))
        .pipe(browserSync.stream());
...
gulp.task ('watch', function (){
    ...
    gulp.watch(source + '/admin_scss/**/*',['admin_sass']);
    gulp.watch(source + '/scss/**/*',['sass']);
    gulp.watch(source + '/vendor/js/**/*',['js-vendor']);
    gulp.watch(source + '/js/**/*',['js']);
    gulp.watch(source + '/admin_js/**/*.js',['js-admin']);
